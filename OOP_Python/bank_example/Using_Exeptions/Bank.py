...
class Bank():
    ...
    def askForValidAccountNumber(self):
        ...
        try:
            ...
            raise AbortTransaction('Account number must be a number')
        if accountNumber not in self.accountsDict:
            raise AbortTransaction('No account number ' + str(accountNumber))
    ...
    def withdraw(self):
        print('*** Withdraw ***')
        oAccount = self.getUsersAccount()
        withdrawAmount = input('Please enter the withdrawal amount:')
    ...
    def getInfo(self):
        ...
        print('We currently have', len(self.accountsDict), 'accounts open')
